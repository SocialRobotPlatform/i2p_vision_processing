    ...
    m_nLastEnergyRefreshTime(NULL)
{
    ...
    ZeroMemory(m_fEnergyDisplayBuffer, sizeof(m_fEnergyDisplayBuffer));
		...
		//i2p connection
	m_client=new ProtectedClient<imi::SoundServiceClient>(IP,imi::g_Inputs_constants.DEFAULT_SOUND_SERVICE_PORT);
}
...
int CAudioBasics::Run(HINSTANCE hInstance, int nCmdShow)
{
    ...
    wc.hIcon         = LoadIconW(hInstance, MAKEINTRESOURCE(IDI_APP3));
...
void CAudioBasics::ProcessAudio()
{
    ...
    DWORD cbRead = 0;
	float fBeamAngle; float fEnergy;float fBeamAngleConfidence;
	...
	imi::AudioLocalization *aAudioLocalization = new imi::AudioLocalization();
	aAudioLocalization->azimuth = fBeamAngle;
	aAudioLocalization->intensity = fEnergy ;
	aAudioLocalization->confidence = fBeamAngleConfidence;
	
		try
		{
			m_client->getClient()->sound("KinectV2",imi::getTimeStamp(),*aAudioLocalization);
		}
